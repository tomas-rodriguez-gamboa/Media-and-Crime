...
data_matching <- data_master %>%
  ...
  mutate(
    treated = case_when(
      quartile == 4 ~ 1,   # cuarto cuartil
      quartile == 1 ~ 0,   # primer cuartil
      TRUE ~ NA_real_
    )
  ) %>%
...
# Export to LaTeX
#-------------------------------------------------------------------------------
out_dir <- "balance_tables_results_Q_medios"
...
#-------------------------------------------------------------------------------
out_dir <- "results_tables_results_Q_medios"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
...
sens_wide <- sensitivity_results %>%
  ...
  mutate(definition = "Top vs. bottom quartile") %>%
