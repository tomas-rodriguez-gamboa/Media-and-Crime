...
#-------------- Block B.1 fix only:                       -
#--------------    Rosenbaum bounds computed on Optimal Matching            -
#--------------    (was: nearest_mahal_exact_prov, contradicting paper).    -
#-------------- Also: path → Mac; tasa_sex_offense → tasa_rape (real name). -
################################################################################
# Preamble
#-------------------------------------------------------------------------------
...
# FIX: drop rows with NAs in any covariate used by the PS / Mahalanobis distance.
# Original code did not do this and matchit() now errors out on NAs in mahvars.
# Affects 2 rows out of 511 (both have NAs in all 9 media-ecosystem variables).
.cov_vars <- c("superficie_km2","genero","indice_envejecimiento",
               "hogares_sin_agua","promedio_personas_viviendas","desague_sin_red",
               "porcentaje_nac_en_extranjeros","tasa_actividad",
               "medio_pandemia","medio_joven","medio_consolidado","medio_tradicional",
               "medio_escrito","medio_audiovisual","medio_redes",
               "extension_geografica","prevencion_delito")
data_master <- data_master[complete.cases(data_master[, .cov_vars]), ]
cat("Effective N after dropping covariate NAs:", nrow(data_master), "\n")
...
# Outcomes
#-------------------------------------------------------------------------------
outcomes <- c("tasa_theft", "tasa_robbery", "tasa_assault", "tasa_rape")
...
purrr::walk(treatment_specs, function(spec) {
  ...
  # (B.1): paper Table A.6 claims bounds come from Optimal Matching, but
  # the original code used nearest_mahal_exact_prov. Corrected to optimal_logit.
  md_mahal <- matched_datasets[["optimal_logit"]]
